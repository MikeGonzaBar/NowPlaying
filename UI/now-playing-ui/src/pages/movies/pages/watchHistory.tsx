...
function WatchHistory() {
  ...
  const [dateRangeAnchor, setDateRangeAnchor] = useState<null | HTMLElement>(
    null,
  );
  ...
  const fetchHistory = async (pageNum: number, isInitial: boolean) => {
    try {
      ...
      const response = await authenticatedFetch(
        getApiUrl(
          `${API_CONFIG.TRAKT_ENDPOINT}/watch-history/?${params.toString()}`,
        ),
      );
        ...
        if (isInitial) {
          ...
          setAllHistoryItems((prev) => {
            ...
            if (newItems.length < (data.history || []).length) {
              console.warn(
                `[WATCH_HISTORY] Filtered out ${(data.history || []).length - newItems.length} duplicate items when loading page ${pageNum}`,
              );
            }
        ...
        for (const movie of moviesToFetch.slice(0, 10)) {
          ...
          if (movie.tmdb_id && !genresMap[movie.tmdb_id]) {
            try {
              ...
              console.error(
                `Error fetching genres for movie ${movie.tmdb_id}:`,
                e,
              );
            }
          }
  ...
  const fetchActivityHeatmap = async () => {
    try {
      ...
      if (!response.ok) {
        console.warn(
          "[WATCH_HISTORY] Failed to fetch activity heatmap:",
          response.status,
          response.statusText,
        );
        return;
      ...
      if (!data || !Array.isArray(data.activity)) {
        console.warn(
          "[WATCH_HISTORY] Invalid activity heatmap response:",
          data,
        );
        return;
      }
  ...
  const fetchTopGenres = async () => {
    try {
      ...
      if (!response.ok) {
        console.warn(
          "[WATCH_HISTORY] Failed to fetch top genres:",
          response.status,
          response.statusText,
        );
        return;
  ...
  const handleItemClick = (item: HistoryItem) => {
    if (item.type === "movie") {
      navigate(item.tmdb_id ? `/movies/${item.tmdb_id}` : "/movies", {
        state: {
          media: {
            movie: {
              ...item,
      ...
      navigate(item.trakt_id ? `/shows/${item.trakt_id}` : "/movies", {
        state: {
          show: {
            id: 0,
            title: item.title,
          ...
          >
            <Box>
              ...
              >
                <Box
                  component="span"
                  sx={{ color: "#ed1c24", fontWeight: 700 }}
                >
            ...
            >
              ...
              >
                <Button
                  ...
                  sx={{
                    ...
                    fontWeight: 700,
                    backgroundColor:
                      filterType === "all" ? "#ed1c24" : "transparent",
                ...
                <Button
                  ...
                  sx={{
                    ...
                    fontWeight: 700,
                    backgroundColor:
                      filterType === "movies" ? "#ed1c24" : "transparent",
                ...
                <Button
                  ...
                  sx={{
                    ...
                    fontWeight: 700,
                    backgroundColor:
                      filterType === "shows" ? "#ed1c24" : "transparent",
              ...
              <Button
                ...
                sx={{
                  ...
                  border: "1px solid #2a2e37",
                  borderColor:
                    dateRange !== "all" ? "rgba(237, 28, 36, 0.55)" : "#2a2e37",
                  borderRadius: 2,
                  fontSize: "10px",
                  fontWeight: 700,
                  color: "#d1d5db",
              ...
              {loading && allHistoryItems.length === 0 ? (
                ...
                Object.entries(groupedHistory).map(([dateKey, items]) => {
                  ...
                  if (itemKeys.length !== uniqueKeys.size) {
                    console.warn(
                      `[WATCH_HISTORY] DUPLICATES IN DATE GROUP ${dateKey}:`,
                      {
                              ...
                              >
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  {item.type === "movie" ? (
                                    ...
                                    >
                                      Movie{" "}
                                      {movieGenres[item.tmdb_id || ""] &&
                                        movieGenres[item.tmdb_id || ""].length > 0
                                    ...
                                    item.episode_info && (
                                      ...
                                      >
                                        S
                                        {item.episode_info.season_number
                                          .toString()
                                          .padStart(2, "0")}
                                        E
                                        {item.episode_info.episode_number
                                          .toString()
                                          .padStart(2, "0")}
                                        {item.episode_info.episode_title
                                          ? ` • ${item.episode_info.episode_title}`
                                          : ""}
                                      </Typography>
                                    )
                  ...
                  {useMemo(() => {
                    ...
                    return gridCells.map((cell, i) => (
                      <Box
                        ...
                        title={
                          cell.date ? `${cell.date}: ${cell.count} watches` : ""
                        }
                        sx={{
                          aspectRatio: "1/1",
                          borderRadius: 0.5,
