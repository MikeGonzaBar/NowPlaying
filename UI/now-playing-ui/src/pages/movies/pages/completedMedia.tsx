  ...
  const fetchCompletedMedia = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const shows = (data.completed_shows || []).map((s: any) => ({
          ...s,
          type: "show" as const,
        }));
              ...
              {allItems.map((item, index) => (
                ...
                >
                  <Box
                    ...
                    role="link"
                    tabIndex={0}
                    aria-label={`${item.title} - view ${item.type} details`}
                    onKeyDown={(event) => {
