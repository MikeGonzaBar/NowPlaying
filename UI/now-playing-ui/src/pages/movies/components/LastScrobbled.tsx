...
function LastScrobbled({ movies, shows }: LastScrobbledProps) {
  ...
  const handleCardClick = (media: any) => {
    if (media.type === "show") {
      const showTrakt = media.show?.ids?.trakt;
      navigate(showTrakt ? `/shows/${showTrakt}` : "/movies", {
        state: {
          show: {
            id: media.show?.id,
      ...
      const movieTmdb = media.movie?.ids?.tmdb;
      navigate(movieTmdb ? `/movies/${movieTmdb}` : "/movies", {
        state: {
          media: media,
      ...
      for (const show of shows) {
        ...
        if (traktId) {
          try {
            ...
            console.error(
              `Error fetching progress for show ${traktId}:`,
              error,
            );
          }
        }
    ...
    >
      <Box
        sx={{
          ...
          height: "4px",
          background:
            "linear-gradient(180deg, rgba(237, 28, 36, 0.2) 0%, rgba(15, 17, 21, 0) 100%)",
        }}
      />
      <CardContent sx={{ p: 3 }}>
        <Box
            ...
            allMedia.map((media, index) => {
              ...
              return (
                <Box
                  ...
                  role="link"
                  tabIndex={0}
                  aria-label={`${getTitle(media)} - view details`}
                  onKeyDown={(event) => {
                  ...
                  sx={{
                    minWidth: 160,
                    cursor: "pointer",
                    "&:focus-visible": { outline: "2px solid #ed1c24", outlineOffset: 3 },
                    "&:hover .poster": {
