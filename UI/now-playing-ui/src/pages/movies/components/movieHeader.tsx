...
const MovieHeader: React.FC<MovieHeaderProps> = ({
  ...
  const [watchedData, setWatchedData] =
    useState<WatchedSeasonsEpisodesResponse | null>(null);
  ...
  const traktId = media
    ? mediaType === "movie"
      ? (media as Movie).movie.ids.trakt
  ...
  const fetchWatchedSeasonsEpisodes = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          "Failed to fetch watched seasons and episodes:",
          response.status,
        );
      }
    } catch (error) {
    ...
    try {
      const tmdbId =
        mediaType === "movie"
          ...
          : (media as Show).show.ids.tmdb;
      const type = mediaType === "movie" ? "movie" : "tv";

      // TMDB is proxied through the backend so the API key never reaches the
      // browser.
      const response = await authenticatedFetch(
        getApiUrl(
          `${API_CONFIG.TRAKT_ENDPOINT}/tmdb-videos/?tmdb_id=${encodeURIComponent(String(tmdbId))}&type=${type}`,
        ),
      );
    ...
    <Box sx={{ paddingLeft: 2.5 }}>
      ...
      {mediaDetails && (
        <Box sx={{ display: "flex", gap: 6, marginTop: 2, width: "100%" }}>
          ...
          <InfoChipSection
            items={mediaDetails?.production_companies || []}
            title="Companies"
          />
          <InfoChipSection
            items={mediaDetails?.spoken_languages || []}
            title="Languages"
          />
          <InfoChipSection
            items={mediaDetails?.production_countries || []}
            title="Countries"
          />
        </Box>
      )}
      {mediaType === "movie" && <TrailerSection trailerKey={trailerKey} />}
      {mediaType === "show" && watchedData && (
        ...
        >
          <ShowEpisodes
            seasons={watchedData.seasons}
            episodes={watchedData.episodes}
          />
