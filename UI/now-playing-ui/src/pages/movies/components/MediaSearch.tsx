...
function MediaSearch() {
  ...
  const searchMedia = async (query: string) => {
    ...
    try {
      const response = await authenticatedFetch(
        getApiUrl(
          `${API_CONFIG.TRAKT_ENDPOINT}/search/?q=${encodeURIComponent(query)}`,
        ),
      );
  ...
  const handleResultClick = async (result: SearchResult) => {
    if (result.type === "episode") {
      ...
      if (!showTraktId || !result.show_title) {
        console.warn(
          "Episode search result is missing parent-show identifiers; ignoring selection",
          result,
        );
        return;
      }
      navigate(
        `/shows/${showTraktId}/seasons/${result.season_number}/episodes/${result.episode_number}`,
        {
          state: {
            show: {
      ...
      if (result.tmdb_id) {
        try {
          const detailResponse = await authenticatedFetch(
            getApiUrl(
              `${API_CONFIG.TRAKT_ENDPOINT}/detail/?type=movie&tmdb_id=${encodeURIComponent(result.tmdb_id)}`,
            ),
          );
      ...
      navigate(result.tmdb_id ? `/movies/${result.tmdb_id}` : "/movies", {
        state: {
          media: storedMedia || {
            movie: {
              title: result.title,
              ...
              ids: {
                trakt: result.id.toString(),
                tmdb: result.tmdb_id?.toString(),
              },
            },
      ...
      if (result.tmdb_id) {
        try {
          const detailResponse = await authenticatedFetch(
            getApiUrl(
              `${API_CONFIG.TRAKT_ENDPOINT}/detail/?type=show&tmdb_id=${encodeURIComponent(result.tmdb_id)}`,
            ),
          );
          if (detailResponse.ok) {
            const detailData = await detailResponse.json();
      ...
      navigate(result.id ? `/shows/${result.id}` : "/movies", {
        state: {
          show: storedShow || {
            id: result.id,
            title: result.title,
  ...
  return (
    <Box sx={{ mb: 3 }}>
      <TextField
        id="media-search"
        fullWidth
        label="Search movies, shows, and episodes"
              ...
              {results.map((result) => (
                ...
                >
                  <Card
                    ...
                    aria-label={`Open ${result.title}${result.type === "episode"
                        ? `, ${result.show_title || "TV show"}`
                        : ""
                      }`}
                    sx={{
                      width: "100%",
                  ...
                  >
                    ...
                    <CardContent sx={{ p: 1.5 }}>
                      ...
                      {result.type === "episode" && result.show_title && (
                        ...
                        >
                          {result.show_title} • S{result.season_number}E
