    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : `Failed to load more ${endpoint.includes("movies") ? "movies" : "shows"}.`;
  ...
  const refresh = async () => {
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : `Failed to refresh ${endpoint.includes("movies") ? "movies" : "shows"}.`;
      setError(errorMessage);
    }
  };
    ...
    const initialFetch = async () => {
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : `Failed to load ${endpoint.includes("movies") ? "movies" : "shows"}.`;
        setError(errorMessage);
      } finally {
        setLoading(false);
      }
