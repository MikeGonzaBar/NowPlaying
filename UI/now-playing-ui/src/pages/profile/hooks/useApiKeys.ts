...
> = {
  ...
  psn: { userId: "", apiKey: "NPSSO Token" },
  xbox: { userId: "XUID", apiKey: "OpenXBL API Key" },
  retroachievements: {
    userId: "RetroAchievements Username",
...
const getRequiredFieldsMessage = (serviceName: string) => {
  const labels = REQUIRED_FIELD_LABELS[serviceName] ?? {
    userId: "User ID",
    apiKey: "API Key",
  };
  if (!labels.userId) {
    return `Please fill in ${labels.apiKey}`;
  }
...
export const useApiKeys = () => {
  ...
  const saveApiKey = async (serviceName: string, keyData?: NewApiKey) => {
    ...
    if (serviceName === "psn") {
      ...
      throw new Error(
        errorData.error || errorData.detail || "Failed to connect PlayStation",
      );
    }
