...
function Games() {
  ...
  const [needsCarouselRecentlyPlayed, setNeedsCarouselRecentlyPlayed] =
    useState(false);
  ...
  const filteredGames = useMemo(() => {
    ...
    return consolidatedGames
      .filter((game) => game.sources.some((source) => source.platform === selectedPlatform))
      .slice(0, 10)
  ...
  const recentAchievements = useMemo(() => {
    ...
    latestPlayedGames.forEach((game) => {
      if (isSteamGame(game) && game.achievements) {
        ...
        game.achievements
          .filter(
            (a: RetroAchievementsAchievement) => a.unlocked && a.unlock_time,
          )
  ...
  const platformAchievements = useMemo(() => {
    ...
    latestPlayedGames.forEach((game) => {
      ...
      if (platform === "psn" && isPsnGame(game) && platforms.psn.psnTrophies) {
        ...
        platforms.psn.psnTrophies.bronze.total +=
          game.total_achievements?.bronze || 0;
        platforms.psn.psnTrophies.silver.unlocked +=
          game.unlocked_achievements?.silver || 0;
        platforms.psn.psnTrophies.silver.total +=
          game.total_achievements?.silver || 0;
        platforms.psn.psnTrophies.gold.unlocked +=
          game.unlocked_achievements?.gold || 0;
        platforms.psn.psnTrophies.gold.total +=
          game.total_achievements?.gold || 0;
        platforms.psn.psnTrophies.platinum.unlocked +=
          game.unlocked_achievements?.platinum || 0;
        platforms.psn.psnTrophies.platinum.total +=
    ...
    Object.keys(platforms).forEach((platform) => {
      if (platformCounts[platform] > 0) {
        ...
        platforms[platform].percentage =
          isNaN(avg) || !isFinite(avg) ? 0 : Math.round(avg);
      } else {
  ...
  const handleReconnect = async () => {
    try {
      ...
      const msg =
        e instanceof Error ? e.message : "Failed to reconnect PlayStation";
      alert(msg);
    } finally {
      setSaving(false);
    }
  ...
  const getPlatformIcon = (platformKey: string): string => {
    switch (platformKey) {
      ...
      case "psn":
        return "/Platforms/playstation.webp";
      case "xbox":
        return "/Platforms/xbox.svg";
      case "retroachievements":
        return "/Platforms/retroachievements.png";
      default:
        return "/Platforms/steam.webp";
    }
  };
  ...
  const canScrollLeftRecentlyPlayed =
    needsCarouselRecentlyPlayed && recentlyPlayedScrollIndex > 0;
  const canScrollRightRecentlyPlayed =
    needsCarouselRecentlyPlayed &&
    recentlyPlayedScrollRef.current &&
    recentlyPlayedScrollRef.current.scrollLeft <
    recentlyPlayedScrollRef.current.scrollWidth -
    recentlyPlayedScrollRef.current.clientWidth -
    10;

  const canScrollLeftRecent100 =
    needsCarouselRecent100 && recent100ScrollIndex > 0;
  const canScrollRightRecent100 =
    needsCarouselRecent100 &&
    recent100ScrollRef.current &&
    recent100ScrollRef.current.scrollLeft <
    recent100ScrollRef.current.scrollWidth -
    recent100ScrollRef.current.clientWidth -
    10;
    ...
    >
      ...
      >
                ...
                <Box sx={{ position: "relative", width: "100%" }}>
                  ...
                  <Box
                    ...
                    sx={{
                      display: "flex",
                      gap: 2,
                      overflowX: needsCarouselRecentlyPlayed
                        ? "auto"
                        : "visible",
                      pb: 1,
                      width: "100%",
                    ...
                    onScroll={(e) => {
                      ...
                      setRecentlyPlayedScrollIndex(
                        Math.round(target.scrollLeft / SCROLL_AMOUNT),
                      );
                    }}
                  >
                    {loading
                      ...
                      : recentlyPlayed.map((game, index) => {
                        ...
                        const achievementPercentage =
                          calculateAchievementPercentage(game);
                        return (
                          <Card
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                {formatMinutesCompact(getPlaytime(game))}
                              </Typography>
                              <LinearProgress
                                variant="determinate"
                              ...
                              >
                                ...
                                <Box
                                  ...
                                  sx={{
                                    ...
                                    cursor: "pointer",
                                    opacity:
                                      selectedPlatform === platform ? 1 : 0.7,
                                    transition:
                        ...
                        >
                          ...
                          {achievement.description &&
                            ...
                            achievement.name.trim() && (
                              ...
                              >
                                {cleanAchievementDescription(achievement.description)}
                              </Typography>
                            )}
                        </Box>
            ...
            <Card>
              <CardContent>
                ...
                <Box
                  sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}
                >
                  {["steam", "psn", "xbox", "retroachievements"].map(
                    (platform) => {
                      const isConfigured =
                        configuredServices.includes(platform);
                      const config =
                            ...
                            >
                              ...
                              >
                                {isNaN(achievementPercentage) ||
                                  !isFinite(achievementPercentage)
                                  ? "—"
        ...
        >
          ...
          <DialogActions
            sx={{ p: 2, borderTop: `1px solid ${zincColors.border}` }}
          >
            <Button
        ...
        >
          ...
          <DialogContent>
            ...
            <Box sx={{ mt: 1 }}>
              <Typography
                sx={{ color: zincColors.muted, fontSize: "12px", mb: 1 }}
              >
                Sign in to PlayStation, open the NPSSO JSON page in the same
                browser, then paste the npsso value.
              </Typography>
