      ...
      try {
        ...
        if (!response.ok) {
          const errorData = (await response
            .json()
            .catch(() => ({}))) as GameDetailResponse;
          throw new Error(
            errorData.error ||
            `Failed to load game details (${response.status})`,
          );
        }
        ...
        if (gameData) {
          ...
          alert(
            `Could not load details for "${game.title}". Please try again.`,
          );
        }
      } catch (error) {
          ...
          <TextField
            ...
            placeholder={
              loadingGameDetails
                ...
                : "Search for games by title or platform..."
            }
            variant="outlined"
            fullWidth
            InputProps={{
