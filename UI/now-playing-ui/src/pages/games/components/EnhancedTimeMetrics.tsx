...
const EnhancedTimeMetrics: React.FC<EnhancedTimeMetricsProps> = ({ game }) => {
  ...
  const maxHours = useMemo(() => {
    ...
    return Math.max(...chartData.map((d) => d.unlocked), 1);
  }, [chartData]);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          ...
          >
            {data.unlocked}{" "}
            {data.unlocked === 1 ? "achievement" : "achievements"} unlocked
          </Typography>
        </Box>
      );
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Achievement activity (Last 30 Days)
        </Typography>
        {chartData.length === 0 ? (
          <Box
            sx={{
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <Bar dataKey="unlocked" radius={[4, 4, 0, 0]}>
                {chartData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={
                      entry.unlocked > maxHours * 0.7
                        ? "rgba(59, 130, 246, 0.9)"
                        : entry.unlocked > maxHours * 0.4
                          ? "rgba(59, 130, 246, 0.7)"
                          : "rgba(59, 130, 246, 0.5)"
                    }
                  />
